...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
  ...
  protected void doFilterInternal(
    ...
    boolean ok = false;
    try {
      ok = (token != null && jwtUtil.validateToken(token));
    } catch (Exception e) {
      log.warn("validateToken threw: {}", e.toString());
    }
      ...
      if (token != null && jwtUtil.validateToken(token)) {
        ...
        if (rolesClaim instanceof List<?> list && !list.isEmpty()) {
          ...
          if (roleSingle != null && !roleSingle.isBlank()) {
            String normalized = roleSingle.startsWith("ROLE_") ? roleSingle : "ROLE_" + roleSingle;
