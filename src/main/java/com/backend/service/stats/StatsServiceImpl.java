...
public class StatsServiceImpl implements StatsService {
    ...
    @Override
    public List<DailyCount> getDauRange(LocalDate startDate, LocalDate endDate, boolean excludeInternal) {
        List<Map<String, Object>> rows = mapper.selectDAURange(startDate, endDate, excludeInternal);
        return rows.stream()
                .map(r -> new DailyCount(
                        toLocalDate(r.get("day")),
                        toLong(r.get("count"))
                ))
                .toList();
    }
    ...
    private static OffsetDateTime toOffsetDateTime(Object v) {
        ...
        if (v instanceof java.util.Date dt)
            return dt.toInstant().atOffset(ZoneOffset.UTC);
        // 문자열로 넘어오는 경우
