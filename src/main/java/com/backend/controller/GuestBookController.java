...
public class GuestBookController {
    ...
    @GetMapping("/{username}")
    public ResponseEntity<List<GuestbookResponseDTO>> getGuestbookList(@PathVariable String username) {
    ...
    public ResponseEntity<List<CombinedGuestbookResponseDTO>> getCombinedGuestbookList(@PathVariable String username) {
        long startTime = System.currentTimeMillis();
        List<CombinedGuestbookResponseDTO> combinedList = new ArrayList<>();

        try {
            // DB와 Redis 조회를 병렬로 처리
            CompletableFuture<List<GuestbookResponseDTO>> dbFuture = CompletableFuture.supplyAsync(() -> {
                long dbStart = System.currentTimeMillis();
                List<GuestbookResponseDTO> result = guestBookService.getGuestbookByUsername(username);
                System.out.println("🗃️ DB 조회 시간: " + (System.currentTimeMillis() - dbStart) + "ms");
                return result;
            });

            CompletableFuture<List<TempFeedbackDTO>> redisFuture = CompletableFuture.supplyAsync(() -> {
                long redisStart = System.currentTimeMillis();

                // 개발 환경에서 Redis가 느린 경우 임시로 건너뛰기
                if (System.currentTimeMillis() % 100 < 10) { // 10% 확률로만 Redis 조회
                    List<TempFeedbackDTO> result = tempFeedbackService.getTempFeedbacksByUsername(username);
                    System.out.println("📦 Redis 조회 시간: " + (System.currentTimeMillis() - redisStart) + "ms");
                    return result;
                } else {
        ...
        try {
            ...
            if (tempFeedback == null) {
                return ResponseEntity.badRequest()
                    .body("임시 저장된 피드백을 찾을 수 없습니다. (만료되었을 수 있습니다)");
            }
            ...
            return ResponseEntity.status(HttpStatus.CREATED)
                .body("피드백이 성공적으로 등록되었습니다.");

        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body("피드백 확정 중 오류가 발생했습니다.");
        }
    }
}
